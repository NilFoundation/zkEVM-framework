#ifndef ZKEMV_FRAMEWORK_BIN_ASSIGNER_INCLUDE_RUNNER_HPP_
#define ZKEMV_FRAMEWORK_BIN_ASSIGNER_INCLUDE_RUNNER_HPP_
...
class single_thread_runner {
  public:
    ...
    single_thread_runner(const std::vector<std::array<std::size_t, 4>>& column_sizes,
        ...
        : m_column_sizes(column_sizes), m_log_level(log_level) {}

    /// @brief Initialize runner predeined account storage
    single_thread_runner(const evmc::accounts& account_storage,
        ...
        : m_account_storage(account_storage),
          m_column_sizes(column_sizes),
          m_log_level(log_level) {}

    /// @brief Execute one block presented as native unsigned char array
    int run(const unsigned char* input_block_data, size_t input_block_size,
        ...
        if (!maybe_input_block.has_value()) {
            std::cerr << "Could not deserilize input block";
            return -1;
        }
    ...
    int run(const data_types::Block& input_block, const std::string& assignment_table_file_name,
            const std::optional<OutputArtifacts>& artifacts) {
        ...
        for (const auto& table_column_sizes : m_column_sizes) {
        ...
        struct evmc_tx_context tx_context = {
            ...
            .block_coinbase =
                to_evmc_address(block_header.m_coinbase),         /**< The miner of the block. */
            ...
            .block_prev_randao = to_uint256be(
                block_header.m_prevrandao), /**< The block previous RANDAO (EIP-4399). */
        ...
        for (const auto& input_msg : input_block.m_inputMsgs) {
            ...
            BOOST_LOG_TRIVIAL(debug)
                << "process CALL message\n  from " << to_str(input_msg.m_info.m_src) << " to "
            ...
            if (acc_block_it != accountBlocks.end()) {
                const std::vector<data_types::Transaction>& transactions =
                    acc_block_it->m_transactions;
            ...
            if (res.status_code == EVMC_SUCCESS) {
                BOOST_LOG_TRIVIAL(debug)
                    << "create_address = " << to_str(res.create_address) << "\n"
  ...
  private:
    ...
    const std::vector<std::array<std::size_t, 4>> m_column_sizes;
    boost::log::trivial::severity_level m_log_level;
};

#endif  // ZKEMV_FRAMEWORK_BIN_ASSIGNER_INCLUDE_RUNNER_HPP_
