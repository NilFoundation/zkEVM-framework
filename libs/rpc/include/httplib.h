...
#ifndef ZKEMV_FRAMEWORK_LIBS_RPC_HTTPLIB_H
#define ZKEMV_FRAMEWORK_LIBS_RPC_HTTPLIB_H
...
namespace httplib {
    ...
    namespace detail {
        ...
        inline uint64_t get_header_value_u64(const Headers &headers, const std::string &key,
                                             size_t id, uint64_t def) {
...
#else
#ifdef SO_REUSEPORT
        setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, reinterpret_cast<const void *>(&yes),
                   sizeof(yes));
#else
        setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const void *>(&yes),
                   sizeof(yes));
#endif
#endif
    }
    ...
    namespace detail {
        ...
        inline const char *mmap::data() const { return static_cast<const char *>(addr_); }
...
#define USE_IF2IP
        ...
        inline bool get_ip_and_port(const struct sockaddr_storage &addr, socklen_t addr_len,
            ...
            if (getnameinfo(reinterpret_cast<const struct sockaddr *>(&addr), addr_len,
                            ipstr.data(), static_cast<socklen_t>(ipstr.size()), nullptr, 0,
                            NI_NUMERICHOST)) {
                return false;
            }
        ...
        inline bool is_chunked_transfer_encoding(const Headers &headers) {
            return compare_case_ignore(get_header_value(headers, "Transfer-Encoding", 0, ""),
                                       "chunked");
        }
        ...
        inline bool range_error(Request &req, Response &res) {
            if (!req.ranges.empty() && 200 <= res.status && res.status < 300) {
                ssize_t contant_len = static_cast<ssize_t>(res.content_length_ ? res.content_length_
                                                                               : res.body.size());
...

        {
            ...
            detail::divide(req.target, '?',
                           [&](const char *lhs_data, std::size_t lhs_size, const char *rhs_data,
                               std::size_t rhs_size) {
                               req.path =
                                   detail::decode_url(std::string(lhs_data, lhs_size), false);
    ...
    inline bool Server::process_request(Stream &strm, bool close_connection,
    ...
    inline Result ClientImpl::Post(const std::string &path, const Headers &headers,
                                   ...
                                   const std::string &content_type) {
        return send_with_content_provider("POST", path, headers, body, content_length, nullptr,
    ...
    inline Result ClientImpl::Put(const std::string &path, const Headers &headers, const char *body,
                                  size_t content_length, const std::string &content_type) {
        return send_with_content_provider("PUT", path, headers, body, content_length, nullptr,
                                          nullptr, content_type, nullptr);
    }
    ...
    inline SSLClient::SSLClient(const std::string &host, int port,
        ...
        if (!client_cert_path.empty() && !client_key_path.empty()) {
            if (!private_key_password.empty()) {
                SSL_CTX_set_default_passwd_cb_userdata(
                    ctx_,
                    reinterpret_cast<void *>(const_cast<char *>(private_key_password.c_str())));
            }
...
#endif  // ZKEMV_FRAMEWORK_LIBS_RPC_HTTPLIB_H
